 ...
 * @version 1.0
 *
 * Modified by a@ajweb.eu to work with Cordova versions 2.8/2.9
 */
public class Twitter extends CordovaPlugin {
	...
	static {
		...
		TWITTER_APPS.put("Twitter2", 		"com.twitter.applib.PostActivity"); // twitter app on Nexus 4 at least (com.twitter.android-2.apk)
	 ...
	 */
	public PluginResult composeTweet(JSONArray args, CallbackContext callbackContext) {
		...
		if(action != null && Arrays.binarySearch(AVAILABLE_METHODS, action) >= 0){
			if(METHOD_IS_TWITTER_AVAILABLE.equals(action)){
                callbackContext.sendPluginResult(
                        new PluginResult(PluginResult.Status.OK, isTwitterAvailable())
                );
				return true;
			}
			
			if(METHOD_COMPOSE_TWEET.equals(action)) {
                callbackContext.sendPluginResult(composeTweet(args, callbackContext));
                return true;
			}
		}
